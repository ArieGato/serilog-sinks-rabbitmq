...
internal sealed class DefaultSendMessageEvents : ISendMessageEvents
{
    ...
    /// <inheritdoc />
    public void Initialize(RabbitMQClientConfiguration configuration) => _routingKey = configuration.RoutingKey;

    /// <inheritdoc />
    public Func<LogEvent, string?> OnGetRoutingKey => _ => _routingKey;

    /// <inheritdoc />
    public Action<LogEvent, IBasicProperties> OnSetMessageProperties => (_, _) => { };
}
